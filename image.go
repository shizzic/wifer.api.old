...
func UploadImage(dir string, c gin.Context) {
	...
	if file.Size < 1000001 {
		...
		if dir != "private" && avatar != nil {
			...
			UpdateDataBaseImages(id, dir, true, true, false, 0)
		} else {
			files, _ := ioutil.ReadDir(path + "/" + dir)
			root := path + "/" + dir + "/" + fmt.Sprint(len(files)+1) + ".webp"
			c.SaveUploadedFile(file, root)
			ConvertResize(root)
			UpdateDataBaseImages(id, dir, false, true, false, len(files)+1)
		}
	}
}
...
func DeleteImage(isAvatar, dir, number string, c gin.Context) {
	...
	if isAvatar == "1" {
		...
		if len(files) > 0 {
			...
			RenameImages(path+"/", "public", id, false, true, false)
		} else {
			RenameImages(path+"/", "public", id, true, false, false)
		}
	} else {
		os.Remove(path + "/" + dir + "/" + number + ".webp")
		RenameImages(path+"/", dir, id, false, true, false)
	}
}
...
func ChangeImageDir(isAvatar, dir, new, number string, c gin.Context) {
	...
	if isAvatar == "1" {
		...
		if len(public) > 0 {
			...
			RenameImages(path+"/", "public", id, true, true, false)
		} else {
			RenameImages(path+"/", "private", id, true, false, true)
		}
	} else {
		...
		if err != nil && new == "public" {
			...
			RenameImages(path+"/", dir, id, true, true, true)
		} else if err == nil {
...
func RenameImages(path, dir, id string, isAvatar, avatar, both bool) {
...
func ReplaceAvatar(dir, num string, c gin.Context) {
	...
	if dir == "private" {
		RenameImages(path+"/", "private", id, false, true, false)
		RenameImages(path+"/", "public", id, false, true, false)
	}
}

// Update info about images in database
func UpdateDataBaseImages(id, dir string, isAvatar, avatar, both bool, quantity int) {
	if both {
